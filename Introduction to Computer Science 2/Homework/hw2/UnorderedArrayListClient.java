...
public class UnorderedArrayListClient {
	...
	public static void main(String[] args) {
		...
		UnorderedArrayList list1 = new UnorderedArrayList(count1);
		UnorderedArrayList list2 = new UnorderedArrayList(count2);
		
		while (read1add.hasNext()) {
			if (read1add.hasNextInt()) {
				list1.insertEnd(read1add.nextInt());
			}
			else read1add.next();
		}
		while (read2add.hasNext()) {
			if (read2add.hasNextInt()) {
				list2.insertEnd(read2add.nextInt());
			}
			else read2add.next();
		}
		
		UnorderedArrayList merged = list1.merge(list2);
		...
		UnorderedArrayList[] lists = merged.split(key);
		list1 = lists[0];
		list2 = lists[1];
